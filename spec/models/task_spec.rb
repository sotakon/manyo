  ...
  describe '検索機能' do
    ...
    context 'scopeメソッドでステータス検索をした場合' do
      it "ステータスに完全一致するタスクが絞り込まれる" do
        expect(Task.search_stutas('未着手')).to include(task)
        expect(Task.search_stutas('未着手')).not_to include(second_task)
        expect(Task.search_stutas('未着手').count).to eq 1
      end
    end

    context 'scopeメソッドでタイトルのあいまい検索とステータス検索をした場合' do
      it "検索キーワードをタイトルに含み、かつステータスに完全一致するタスク絞り込まれる" do
        expect(Task.search_name('task').search_stutas('未着手')).to include(task)
        expect(Task.search_name('task').search_stutas('未着手')).not_to include(second_task)
        expect(Task.search_stutas('未着手').count).to eq 1
      end
    end
  end
